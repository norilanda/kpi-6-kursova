...
3. We take the outputs and compare them with the created tool (detector.py) which shows which tests may be flaky depending on their results in every run (if in one run a test passed but in another one it fails - it is flaky, if it passes/fails in every run - it is considered as NOT flaky)
